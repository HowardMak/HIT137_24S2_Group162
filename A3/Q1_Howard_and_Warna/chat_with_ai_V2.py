...
class Application(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.ai_mode = 'Casual'  # Default AI Mode
    ...
    def adjust_app_position(self):
        """
        Center the window on the screen and set the minimum window size
        """
        window_width = 900
        window_height = 600
        ...
        self.master.minsize(window_width, window_height)  # Set minimum window size
    ...
    def create_widgets(self):
        ...
        self.top_ribbon.grid(row=0, column=0, columnspan=4, sticky="ew", padx=5, pady=5)
        ...
        self.chat_area.grid(row=4, column=0, columnspan=3, padx=10, pady=5, sticky='nsew')
        ...
        self.bottom_ribbon.grid(row=7, column=0, columnspan=4, sticky="ew", padx=5, pady=5)
        ...
        # --- Additional Buttons ---
        self.save_button = tk.Button(self.master, text="Save Chat", bg=button_bg, fg=button_fg, font=("Helvetica Neue", 12, "bold"),
                                     relief="flat", activebackground="#145D9E", activeforeground=button_fg, command=self.save_chat,
                                     width=15, height=2, borderwidth=0, highlightthickness=0)
        self.save_button.grid(row=6, column=0, padx=10, pady=10, sticky='w')

        self.delete_button = tk.Button(self.master, text="Delete Chat", bg=button_bg, fg=button_fg, font=("Helvetica Neue", 12, "bold"),
                                       relief="flat", activebackground="#145D9E", activeforeground=button_fg, command=self.delete_selected_chat,
                                       width=15, height=2, borderwidth=0, highlightthickness=0)
        self.delete_button.grid(row=7, column=0, padx=10, pady=10, sticky='w')

    def connect_to_api(self):
        """
        Connect to the Gemini API using the API Key entered by the user
        """
        api_key = self.api_key.get("1.0", "end-1c").strip()
        if api_key:
            try:
                self.model = genai.from_api_key(api_key)
                self.load_api_key(api_key)
                messagebox.showinfo("Connection Successful", "Connected to Gemini API successfully.")
            except ValueError as e:
                messagebox.showerror("API Key Error", str(e))
        else:
            messagebox.showerror("API Key Error", "Please enter a valid API Key.")

    def open_api_website(self):
        """
        Open the Gemini API website to allow the user to get the API key
        """
        webbrowser.open("https://console.generativeai.google.com/")
    ...
    def save_chat(self):
        ...
        if not self.chat_area.get("1.0", tk.END).strip():
            messagebox.showwarning("No Chat", "There is no chat to save.")
            return
        
        chat_filename = os.path.join(CURRENT_DIR, f"chat_{self.current_chat_key}.txt")
        with open(chat_filename, "w") as f:
            f.write(self.chat_area.get("1.0", tk.END).strip())
        messagebox.showinfo("Chat Saved", f"Chat saved to {chat_filename}")
    ...
    def load_api_key(self):
        """
        Load the API key from file if exists
        """
